/**
 * Tests for UnestimatedPanel.vue component - following TDD practices.
 */
...
describe('UnestimatedPanel', () => {
  const mockUnestimatedIssues = [
    {
      key: 'RHOAIENG-200',
      summary: 'New task without estimate',
    ...
    {
      key: 'RHOAIENG-201',
      summary: 'Another unestimated issue',
      url: 'https://issues.redhat.com/browse/RHOAIENG-201',
      storyPoints: null,
      status: 'In Progress',
      completed: false
    }
  ...
  it('renders nothing when issues array is empty', () => {
    const wrapper = mount(UnestimatedPanel, {
      props: { issues: [] }
    })
  ...
  it('renders amber warning banner when there are unestimated issues', () => {
    const wrapper = mount(UnestimatedPanel, {
      props: { issues: mockUnestimatedIssues }
    })

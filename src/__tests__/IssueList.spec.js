/**
 * Tests for IssueList.vue component - following TDD practices.
 */
...
describe('IssueList', () => {
  const mockIssues = [
    {
      key: 'RHOAIENG-123',
      summary: 'Fix login bug that prevents users from accessing dashboard',
    ...
    },
    {
      ...
      url: 'https://issues.redhat.com/browse/RHOAIENG-456',
      storyPoints: 3,
      status: 'Done',
      completed: true
    },
    {
      key: 'RHOAIENG-789',
      summary: 'Unestimated task without story points',
      url: 'https://issues.redhat.com/browse/RHOAIENG-789',
      storyPoints: null,
      status: 'To Do',
      completed: false
    }
  ...
  describe('expandable mode (default)', () => {
    it('is collapsed by default', () => {
      ...
      // Should show toggle button but not issue rows
  ...
  describe('non-expandable mode', () => {
    it('shows all issues immediately when expandable=false', () => {
      const wrapper = mount(IssueList, {
        props: { issues: mockIssues, expandable: false }
      })
  ...
  describe('issue row rendering', () => {
    it('shows completed checkmark for completed issues', async () => {
      ...
      const rows = wrapper.findAll('[data-testid="issue-row"]')
      const completedRow = rows[1] // RHOAIENG-456 is completed
    ...
    it('shows amber highlight for unestimated issues', async () => {
      ...
      const rows = wrapper.findAll('[data-testid="issue-row"]')
      const unestimatedRow = rows[2] // storyPoints: null
